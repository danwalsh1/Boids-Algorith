...
let alignmentSlider, seperationSlider, cohesionSlider, maxSpeedSlider, maxForceSlider, alignmentRadiusSlider, seperationRadiusSlider, cohesionRadiusSlider, canvasWidthSlider, canvasHeightSlider, playCheckbox, quadtreeCheckbox;
...
function setup(){
	...
	// Two checkboxes to control play/pause and whether or not to show the QuadTree grid.
	playCheckbox = createCheckbox('', true);
	playCheckbox.parent('playCheckboxContainer');
	quadtreeCheckbox = createCheckbox('', false);
	quadtreeCheckbox.parent('quadtreeCheckboxSliderContainer');
...
function draw(){
	...
	if(playCheckbox.checked()){
		...
		// Take a copy of the boids array.
		let boidsSnapshot = boids.slice();
		
		// Put the Boids into a Quadtree.
		let boundary = new Rectangle(width/2,height/2,width/2,height/2);
		let quadtree = new QuadTree(boundary, 10);
		for(let boid of boidsSnapshot){
		...
		for(let boid of boids){
			// Boids now update at "the same point in time"
			// Boids now update using the quadtree, built on the above snapshot.
			boid.flock(quadtree);
			boid.update();
			boid.borders();
			boid.render();
